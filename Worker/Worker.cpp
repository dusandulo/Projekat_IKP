...
int main()
{
    ...
    while (true) {
        ...
        if (iResult != SOCKET_ERROR)	// Check if message is successfully received
        {
            ...
            int receivedNumber = atoi(dataBuffer + 9);  // Convert the received string to an integer
            char temp[] = "";

            if (receivedNumber <= 350) { // Green zone

                int multipliedNumber = receivedNumber * 6; // Calculate bill for green zone

                sprintf(temp, "You are in green zone, and your bill for spending %d kw is: %d RSD", receivedNumber, multipliedNumber);
            }
            else if (receivedNumber > 350 && receivedNumber <= 1600) { // Blue zone

                int multipliedNumber = receivedNumber * 9; // Calculate bill for blue zone

                sprintf(temp, "You are in blue zone, and your bill for spending %d kw is: %d RSD", receivedNumber, multipliedNumber);
            }
            else if (receivedNumber > 1600) { // Red zone

                int multipliedNumber = receivedNumber * 18; // Calculate bill for red zone

                sprintf(temp, "You are in red zone, and your bill for spending %d kw is: %d RSD", receivedNumber, multipliedNumber);
            }

            int multipliedNumber = receivedNumber * 6; // Calculate the multiplied number
            ...
            sprintf(clientId, "Client%d:", atoi(dataBuffer + 7));
            strcpy(dataBuffer2 + 1, successStr);
            strcpy(dataBuffer2 + 1 + strlen(dataBuffer2 + 1), clientId);
